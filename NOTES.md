...
Добавлено 2026-09-22. Раньше действовало правило «новый тул или изменённая
схема — только рестарт MCP-клиента». Теперь это почти всегда не нужно.

**Как это работает.** `list_tools()` читает `ext.TOOL_SCHEMAS` в момент
вызова, а `importlib.reload` меняет модуль на месте — значит свежий список
уже готов, клиенту остаётся только переспросить. Не хватало двух вещей, обе
добавлены в `server.py`:

- в `create_initialization_options` передаётся
  `NotificationOptions(tools_changed=True)` — это объявляет способность
  `tools.listChanged`;
- после успешного `reload_api` отправляется
  `session.send_tool_list_changed()`.

В ответ `reload_api` теперь дописывает `| tools/list_changed sent`. Если
уведомление отправить не удалось, пишет об этом прямо и советует рестарт —
молча деградировать тут нельзя, иначе решишь, что тул не появился из-за
ошибки в коде.

**Проверено 2026-09-22, и результат отрицательный для этого клиента.**
Уведомление уходит без ошибки, сервер отдаёт все 55 тулов — но в уже
открытом чате Claude Desktop новый тул всё равно недоступен
(`No such tool available`). Приложение фиксирует список инструментов на
момент старта разговора и по `tools/list_changed` его не пересобирает.

Что из этого следует практически:

- **новый тул виден в НОВОМ чате**, а не после рестарта сервера. Рестарт
  сам по себе не помогает: список привязан к разговору, а не к процессу;
- в текущем чате новый код всё равно доступен — через
  `execute_python` + `importlib.reload(ext)` и прямой вызов
  `ext.t_имя(sw, {...})`. Так и отлаживался весь этап 1;
- отправка `tools/list_changed` оставлена: она корректна по протоколу и
  сработает в клиентах, которые её уважают (Cursor, VS Code). Для Claude
  Desktop это пока no-op.

`solidworks_mcp.toolsets` добавлен в `_RELOAD_ORDER`, так что правки
разделов тоже подхватываются живьём.

**Когда рестарт всё же нужен:** правки самого `server.py` — `reload_api`
его сознательно не перезагружает (иначе пришлось бы пересоздавать объект
`Server` вместе со всеми зарегистрированными хендлерами). Рестарт при этом
поднимает процесс с новым кодом, но список тулов в уже открытом чате не
обновляет — для этого нужен новый чат.

Ещё одно наблюдение оттуда же: после того как процессы убиты, клиент
поднимает сервер **лениво, на первом же вызове тула**, сам по себе он этого
не делает. А `session_connectors_status` в этот момент продолжает
показывать `connected` и старый `tool_count` — это кэш приложения, не живой
опрос. Судить о состоянии по нему нельзя, надо смотреть процессы
(`restart_mcp.ps1 -List`).

### scripts/restart_mcp.ps1 (и .cmd для двойного клика)

Убивает процесс сервера, чтобы клиент поднял его заново. SolidWorks не
трогает: COM-коннект восстановит следующий `connect_solidworks`, открытые
документы остаются.

**Процессов несколько.** Приложение поднимает сервер на каждую сессию, а не
один на всех: на 2026-09-22 висело **четыре** процесса, парами с
одинаковым временем старта, самые старые — 16 часов. Поэтому у скрипта есть
`-List` (посмотреть, с PID и временем старта) и `-Id <PID>` (убить один).
Без аргументов бьёт все — чужие сессии поднимут свои при следующем вызове,
но то, что выполняется там прямо сейчас, прервётся.

**Важное следствие, поправка к разбору сравнения с SW+.** Раньше было
записано, что гонок внутри сервера нет, потому что всё сериализовано в
одном потоке одного процесса. Это верно *для одного процесса*, но процессов
несколько, и все они маршалят COM в один и тот же SLDWORKS.exe. То есть
межпроцессные гонки возможны: две сессии, работающие с CAD одновременно,
друг о друге не знают. Ещё один довод за пробу занятости из этапа 2.

## Проба занятости: отказ вместо зависания

Добавлено 2026-09-22 (этап 2). `ext.sw_busy()` + вызов в `call_tool`.

**Что чинит.** Каждый COM-вызов маршалится в SLDWORKS.exe снаружи,
синхронно, в потоке event loop. Если в SolidWorks открыто модальное окно
или идёт долгое перестроение, вызов **не падает — он блокируется**, и
вместе с собой вешает весь сервер: ни один другой тул не выполнится, отмена
не обработается, клиент отвалится по таймауту без единого внятного слова.
Это худший режим отказа внешнего сервера, и у in-process надстройки его нет
в принципе.

**Как.** Приём подсмотрен в `panel.dll` SW+ (строки `SMTO_ABORTIFHUNG`,
`SMTO_BLOCK`, `SendMessageTimeout`) — и работает снаружи ровно так же, как
изнутри, потому что ему нужен только хэндл окна:
`SendMessageTimeout(hwnd, WM_NULL, SMTO_BLOCK|SMTO_ABORTIFHUNG, timeout)`.
Отзывчивое окно отвечает мгновенно, занятое не отвечает, и API возвращает
`ERROR_TIMEOUT` вместо блокировки.

Замерено на живом SolidWorks: **0.21–0.30 мс** на пробу в простое. Порог по
умолчанию 1200 мс (`BUSY_TIMEOUT_MS`), переопределяется переменной
`SW_MCP_BUSY_TIMEOUT_MS`, `0` полностью выключает пробу.

**Хэндл кэшируется намеренно.** Достаётся через `Frame().GetHWndx64()` —
а это сам по себе COM-вызов, который завис бы ровно по той причине, которую
мы пытаемся обнаружить. Кэш валидируется через `IsWindow`, поэтому
перезапущенный SolidWorks подхватывается со следующего вызова, а не
опрашивается мёртвый хэндл вечно. Проверено: после подмены кэша на
заведомо неверный хэндл следующий `sw_hwnd()` вернул правильный.

`GetHWnd()` и `GetHWndx64()` на этой сборке возвращают одно и то же
значение; код пробует x64-вариант первым и откатывается на обычный.

**Проба никогда не бросает исключение и не блокирует дольше таймаута.**
Если решить не удалось — возвращает «готов», а не «занят»: проба, которая
сама становится источником отказов, хуже отсутствия пробы.

**Исключения (`BUSY_EXEMPT` в server.py).** `connect_solidworks`,
`get_solidworks_info`, `lookup_api_signature`, `lookup_api_constant`,
`reload_api`, `set_units` — они либо не трогают живой документ, либо это
именно то, чем разбираются, когда всё встало. Они обязаны работать как раз
тогда, когда SolidWorks завис.

**Не заменяет `IMessageFilter`.** Проба ловит состояние ДО вызова. Если
SolidWorks станет модальным уже в момент выполнения вызова, поможет только
`CoRegisterMessageFilter` с повтором на `RPC_E_CALL_REJECTED`. Это
следующий шаг, если такое начнёт случаться на практике.

Смежное наблюдение, усиливающее нужность пробы: приложение поднимает **по
серверу на сессию**, и все они маршалят COM в один и тот же SLDWORKS.exe
(см. § Перезагрузка сервера). То есть источником «занятости» может быть не
только человек за мышкой, но и другая сессия Claude.
