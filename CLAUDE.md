...
- **Правки кода: `reload_api`** перечитывает `automation/*`, `ext.py` и
  ...
  клиенту `tools/list_changed`, поэтому **новые тулы и изменённые схемы
  подхватываются живьём** — рестарт клиента для этого больше не нужен.
  Новое — писать в `ext.py` (он перезагружаемый). Рестарт остаётся нужен
  только при правках самого `server.py`: для этого
